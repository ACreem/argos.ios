...
- (void)loadData
{
    ...
    if (!self.paginator) {
        ...
        [self.paginator setCompletionBlockWithSuccess:^(RKPaginator *paginator, NSArray *objects, NSUInteger page) {
            ...
            // For Bookmarked and Watching, a 404 on the first page means
            // the user has not yet bookmarked or watched anything yet (respectively).
            if (statusCode == 404 && paginator.currentPage == 1) {
                ...
                if ([weakSelf.stream isEqualToString:kArgosBookmarkedStream]) {
                    noteLabel.text = @"You don't have any bookmarks yet.";
                } else if ([weakSelf.stream isEqualToString:kArgosWatchingStream]) {
                    noteLabel.text = @"You aren't watching any stories yet.";
                }
                noteLabel.font = [UIFont titleFontForSize:14.0];
                noteLabel.textAlignment = NSTextAlignmentCenter;
                noteLabel.backgroundColor = [UIColor actionColor];
                noteLabel.textColor = [UIColor whiteColor];
                [weakSelf.collectionView addSubview:noteLabel];
                [weakSelf.collectionView bringSubviewToFront:noteLabel];
            } else {
